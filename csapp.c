 ...
 *
 * 2016년 2월 수정:
 *   - open_clientfd와 open_listenfd가 더 자연스럽게 실패하도록 수정
 *
 * 2014년 8월 수정: 
 ...
 *     inet_ntoa와 inet_aton은 더 이상 권장되지 않음
 *
 * 2014년 7월 수정:
...
void app_error(char *msg) /* 응용 프로그램 오류 */
{
    fprintf(stderr, "%s\n", msg);
    exit(0);
}
...
handler_t *Signal(int signum, handler_t *handler) 
{
    ...
    action.sa_flags = SA_RESTART; /* 가능하면 시스템 호출 다시 시작 */
...
/*************************************************************
 * Sio(시그널 안전 I/O) 패키지 - 시그널 핸들러 안에서도 안전하게
 * 사용할 수 있는 단순한 재진입 출력 함수
 *************************************************************/

/* 내부 sio 함수 */

/* $begin sioprivate */
/* sio_reverse - 문자열 뒤집기(K&R에서 가져옴) */
...
/* sio_ltoa - long 값을 b진수 문자열로 변환(K&R에서 가져옴) */
...
/* sio_strlen - 문자열 길이 반환(K&R에서 가져옴) */
...
ssize_t sio_putl(long v) /* long 값 출력 */
{
    ...
    sio_ltoa(v, s, 10); /* K&R의 itoa()를 바탕으로 함 */  //line:csapp:sioltoa
    return sio_puts(s);
}
 ...
 * DNS 인터페이스 래퍼
 *
 * 참고: 이 함수들은 스레드 안전하지 않아 더 이상 권장되지 않습니다.
...
ssize_t rio_readn(int fd, void *usrbuf, size_t n) 
{
	...
	if ((nread = read(fd, bufp, nleft)) < 0) {
	    if (errno == EINTR) /* 시그널 핸들러 때문에 중단된 경우 */
...
/*
 * rio_writen - n바이트를 견고하게 쓰기(버퍼 없음)
 */
/* $begin rio_writen */
...
/* 
 * rio_read - Unix read() 함수용 래퍼로, 내부 버퍼에서 사용자 버퍼로
 *    min(n, rio_cnt) 바이트를 옮긴다. 여기서 n은 사용자가 요청한 바이트 수,
 *    rio_cnt는 내부 버퍼에 아직 읽지 않은 바이트 수이다.
 *    함수 진입 시 내부 버퍼가 비어 있으면 read()를 호출해 채운다.
 */
/* $begin rio_read */
...
/*
 * rio_readnb - n바이트를 견고하게 읽기(버퍼 사용)
 */
/* $begin rio_readnb */
...
/* 
 * rio_readlineb - 텍스트 한 줄을 견고하게 읽기(버퍼 사용)
 */
/* $begin rio_readlineb */
...
int open_clientfd(char *hostname, char *port) {
    ...
    hints.ai_flags |= AI_ADDRCONFIG;  /* 연결 시 권장 옵션 */
    if ((rc = getaddrinfo(hostname, port, &hints, &listp)) != 0) {
        fprintf(stderr, "getaddrinfo failed (%s:%s): %s\n", hostname, port, gai_strerror(rc));
        return -2;
    }
  
    /* 성공적으로 연결할 수 있는 주소를 찾을 때까지 목록 순회 */
    for (p = listp; p; p = p->ai_next) {
        ...
        if (connect(clientfd, p->ai_addr, p->ai_addrlen) != -1) 
            break; /* 성공 */
        if (close(clientfd) < 0) { /* 연결 실패, 다른 후보 시도 */  //line:netp:openclientfd:closefd
...
/*  
 * open_listenfd - port에서 대기용 소켓을 열고 반환한다.
 *     이 함수는 재진입 가능하며 프로토콜에 독립적이다.
 *
 *     오류 발생 시 반환값:
 *       - getaddrinfo 오류면 -2
...
int open_listenfd(char *port) 
{
    ...
    hints.ai_flags = AI_PASSIVE | AI_ADDRCONFIG; /* 임의의 IP 주소에서 대기 */
    ...
    /* 바인드할 수 있는 주소를 찾을 때까지 목록 순회 */
    for (p = listp; p; p = p->ai_next) {
        /* 소켓 디스크립터 생성 */
        if ((listenfd = socket(p->ai_family, p->ai_socktype, p->ai_protocol)) < 0) 
            continue;  /* 소켓 생성 실패, 다음 후보 시도 */

        /* bind 시 "Address already in use" 오류 완화 */
        ...
        if (bind(listenfd, p->ai_addr, p->ai_addrlen) == 0)
            break; /* 성공 */
        if (close(listenfd) < 0) { /* 바인드 실패, 다음 후보 시도 */
